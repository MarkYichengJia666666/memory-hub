...
@Service
@Slf4j
public class LoanRetrievalRiskProcessor extends BaseRiskProcessor {
  ...
  protected LoanUserRiskType getLoanUserRiskType() {
    return LoanUserRiskType.LOAN_RETRIEVAL;
  }

  /**
   * 前置判断，上次的trace数据必须符合回捞一次风控的风控类型要求，并且状态必须是拒绝的操作才可以
   *
  ...
  protected void assertBeforeSubmit(LoanUserCreditsInfoRecord creditsInfoRecord, RiskProcessParam param) {
    LoanAssertion.assertAccountAndAppInCreditsStatus(creditsInfoRecord, LoanCreditsStatus.REJECTED);
    ...
    if (loanUserRiskTraceVO == null) {
      throw EcException.error("loan user account {} is not exists in last finish credits risk trace!!!", creditsInfoRecord.getLoanAccountId());
    }
    if (!isValidCreditsStatusForSubmitRetrieval(loanUserRiskTraceVO.creditsStatus)
        || !LoanUserRiskType.getLoanRetrievalPreRiskType().contains(loanUserRiskTraceVO.riskType)) {
      throw EcException.error("loan user account {} is not rejected and preconditions loan retrieval risk type in last finish credits [{}] risk trace!!!",
          creditsInfoRecord.getLoanAccountId(), StringUtils.join(LoanUserRiskType.getLoanRetrievalPreRiskType(), ","));
    }
  }
