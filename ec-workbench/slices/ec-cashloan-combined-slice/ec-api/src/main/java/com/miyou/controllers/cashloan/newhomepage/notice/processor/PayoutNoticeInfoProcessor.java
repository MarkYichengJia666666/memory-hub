 ...
 */
@Service
public class PayoutNoticeInfoProcessor extends AbstractNoticeInfoProcessor{
  ...
  public void doProcessorForOldHomepage(NoticeListResponse noticeInfo, HomePageContext homePageContext) {
    noticeInfo
        .setRedirectUrl(homepageV5Config.getBankCardRedirectUrl())
        .setColor(FontColor.GREEN)
        .setData(Lists.newArrayList(TT.gen("订单打款失败，请确认你的银行卡信息是否正确")));
  }

  @Override
  protected HomepageNoticeProcessorType getNoticeProcessorType() {
    return HomepageNoticeProcessorType.PAYOUT_NOTICE_INFO_PROCESSOR;
  }
}