...
@Slf4j
@Component
public class BillPageJumpForOverdueProcessor extends AbstractHomepageUserInfoV3Processor {
  @Autowired
  ...
  public void process(PageUserInfoV3Response fieldsInfo, HomePageContext homePageContext) {
    try {
      if (!jumpBillPageTool.canJumpBillPageForOverdue(homePageContext.getUserId())) {
        return;
      }
  ...
  public HomepageUserInfoV3ProcessorType getProcessorType() {
    return HomepageUserInfoV3ProcessorType.AUTO_JUMP_BILL_PAGE_FOR_OVERDUE;
  }
}
